...
changeNameButton.addEventListener("click", async () => {
  //Get new name
  const name = await GlobalDialogManager.ShowPromptAsync("Enter name", "Enter new password for sheet:", null, "text", { useMinMaxAsLen: true, min: "1", max: "64", placeholder: changeNameButton.getAttribute("value") as string, presetValue: changeNameButton.getAttribute("value") as string });
  if (name == null) {
...
for (const button of document.getElementsByClassName("archiveButton")) {
  button.addEventListener("click", async () => {
    ...
    formData.set("sheet", params.get("id") as string);
    const [ok, resp] = await SendPOSTDataToServerAsync("./manageSheet.php", formData);
    if (ok) {
